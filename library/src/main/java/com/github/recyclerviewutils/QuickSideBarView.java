...
public class QuickSideBarView extends View {
    ...
    private final ArrayList<String> hasLetters = new ArrayList<>();
    ...
    private final int[] textStyles = {Typeface.NORMAL, Typeface.BOLD, Typeface.ITALIC, Typeface.BOLD_ITALIC};
    private int textStyle = Typeface.NORMAL;
    private int textChooseStyle = Typeface.NORMAL;
    private int textHasStyle = Typeface.NORMAL;
        ...
        if (orientation == VERTICAL) {
            ...
            for (int i = 0; i < letters.size(); i++) {
                ...
                if (i == choose) {
                    paint.setColor(textColorChoose);
                    paint.setTextSize(textSizeChoose);
                    paint.setTypeface(Typeface.create(Typeface.DEFAULT, textStyles[textChooseStyle]));
                } else if (hasLetters.contains(s)) {
                    paint.setColor(textColorHas);
                    paint.setTextSize(textSizeHas);
                    paint.setTypeface(Typeface.create(Typeface.DEFAULT, textStyles[textHasStyle]));
                } else {
                    paint.setColor(textColor);
                    paint.setTextSize(textSize);
                    paint.setTypeface(Typeface.create(Typeface.DEFAULT, textStyles[textStyle]));
                }
                rect.setEmpty();
                paint.getTextBounds(s, 0, s.length(), rect);
