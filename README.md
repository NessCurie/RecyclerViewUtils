## RecyclerViewUtils
支持快速完成单类型item的RecyclerView;
便利完成多type类型item的RecyclerView;
RecyclerView分割线的实现,支持GridLayoutManager的横竖都显示;
使用Google官方ItemTouchHelper完成侧拉删除和条目拖动和指定控件触发拖动;
封装item的长按和点击事件;
修改SwipeRefreshLayout同时支持下拉刷新和上拉加载更多,兼容条目拖动.

For more information please see [Möbius Strip World](https://nesscurie.github.io/)
